...
class YoloTinyNet(Net):
  ...
  def iou(self, boxes1, boxes2):
    ...
    """
    #因tensorflow升级，pack已经取消，需要使用unstack
    boxes1 = tf.unstack([boxes1[:, :, :, 0] - boxes1[:, :, :, 2] / 2, boxes1[:, :, :, 1] - boxes1[:, :, :, 3] / 2,
                      boxes1[:, :, :, 0] + boxes1[:, :, :, 2] / 2, boxes1[:, :, :, 1] + boxes1[:, :, :, 3] / 2])
    boxes1 = tf.transpose(boxes1, [1, 2, 3, 0])
    boxes2 =  tf.unstack([boxes2[0] - boxes2[2] / 2, boxes2[1] - boxes2[3] / 2,
  ...
  def body1(self, num, object_num, loss, predict, labels, nilboy):
    ...
    temp = tf.cast(tf.unstack([center_y, self.cell_size - center_y - 1, center_x, self.cell_size -center_x - 1]), tf.int32)
